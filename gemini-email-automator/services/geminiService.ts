...
async function handleWebScraping(prompt: string): Promise<AssistantMessageData> {
    const scrapeTarget = prompt.trim();

    if (!scrapeTarget) {
        return { text: "Please provide a URL or a query to scrape." };
    }

    try {
        // Call the backend scraping proxy
        const response = await axios.post('http://localhost:3001/api/scrape', { url: scrapeTarget });
        const { emails, phones, addresses } = response.data;

        if (!emails?.length && !phones?.length) {
            return { text: `⚠️ I scraped "${scrapeTarget}" but couldn't find any contact information.` };
        }

        // Extract company name from the URL for fallback
        let companyName = 'Scraped Company';
        try {
            const url = new URL(scrapeTarget);
        ...
        const scrapedItem = {
            id: generateUUID(),
            full_name: `${companyName} Inquiry`,
            company: companyName,
            work_email: emails?.[0] || '',
            personal_emails: emails?.slice(1) || [],
            websites: phones?.map((phone: string) => ({ type: 'phone', url: phone })) || [],
            source_details: addresses?.join(' | ') || 'N/A',
            source: 'real_web_scrape',
            query: scrapeTarget,
        };

        const responseText = `✅ I scraped "${scrapeTarget}" and found ${emails.length} email(s) and ${phones.length} phone number(s).`;
        return { text: responseText, data: [scrapedItem] };

    } catch (error: any) {
        console.error("Error calling scraping proxy:", error);
        const detail = error.response?.data?.error || error.message;
        return { text: `Sorry, the scraping process failed. Please ensure the backend server is running and the URL is correct. Error: ${detail}` };
    }
}
